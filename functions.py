...
def get_h_air_wd(T_wd):
    ...
    #考察:引用:由田、式9,10、加熱面,非加熱面同じ式のため、非加熱面ではあるが空気と木材の式10を採用


def get_lambda_wd(T_wd, T1_wd=20, lambda1=0.09, rho_wd_0=RHO_WD_0):
    """木材の温度から熱伝導率を求める関数
    ...
    #考察:引用元の式がおそらく違うため、改編版引用:木材の物理p63(4-12)


def get_lambda_wd_0(T_wd, T1_wd=20, rho_wd_0=RHO_WD_0):
...
def get_rho_c_wd(T_wd):
    ...
    Returns:
        J/m³*K: 密度ρ*比熱c
    """
    lambda_wd = get_lambda_wd(T_wd)
    return lambda_wd / ALPHA_WD


def get_rho_c_wd_0(T_wd):
    ...
    Returns:
        J/m³*K: 密度ρ*比熱c
    """
    lambda_wd = get_lambda_wd_0(T_wd)
    return lambda_wd / ALPHA_WD
